...
def main():
    ...
    z_bg_color = (0, 0, 0)  # black for Z background
    ...
    try:
        while True:
            ...
            for box in results.boxes:
                ...
                # background rect inside box, top-left
                lx1, ly1 = x1 + 2, y1 + 2
                lx2, ly2 = lx1 + cls_w + 4, ly1 + cls_h + 4
                cv2.rectangle(img, (lx1, ly1), (lx2, ly2), box_color, cv2.FILLED)
                # text
                cv2.putText(img, cls_name, (lx1 + 2, ly2 - 2),
                            font, fs, (255,255,255), thk, cv2.LINE_AA)

                # — XYZ OVERLAY INSIDE BOX, BELOW CLASS LABEL —
                labels = [f"X:{X:.2f}", f"Y:{Y:.2f}", f"Z:{Z:.2f}m"]
                font2, fs2, thk2 = cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1
                spacing = 6
                # starting position just below class label
                tx = lx1 + 2
                ty = ly2 + cls_h + 4
                for lbl in labels:
                    ...
                    # text
                    cv2.putText(img, lbl, (tx, ty),
                                font2, fs2, (255,255,255), thk2)
                    tx += w + spacing

            cv2.imshow('YOLO + Depth + Class + XYZ', img)
