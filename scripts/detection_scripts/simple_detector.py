    ...
    try:
        while True:
            ...
            for box in results.boxes:
                ...
                spacing = 6
                tx_start, ty_start = lx1 + 2, ly2 + cls_h + 8
                ...
                # — CONFIDENCE OVERLAY BELOW XYZ —
                conf_text = f"Conf:{conf_val:.2f}"
                (cw, ch), _ = cv2.getTextSize(conf_text, font2, fs2, thk2)
                ty_conf = ty_start + ch + 6
                cv2.rectangle(
                    img,
                    (tx_start - 2, ty_conf - ch - 2),
                    (tx_start + cw + 2, ty_conf + 2),
                    color, cv2.FILLED
                )
                cv2.putText(
                    img, conf_text,
            ...
            cv2.imshow('YOLO + Depth + Smoothed XYZ + Confidence', img)
