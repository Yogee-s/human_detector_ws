...
class HumanPublisher:
    def __init__(self, slam_point):
        ...
        # smoothing buffer for nearest index
        self.nearest_history = deque(maxlen=7)
    ...
    def find_nearest_person(self, detections):
        ...
        for i, det in enumerate(detections):
            d = math.hypot(det['x'] - self.slam_point[0],
                           det['y'] - self.slam_point[1])
            if d < min_d:
    ...
    def inference_loop(self):
        ...
        while self.running:
            ...
            if res.keypoints is not None:
                ...
                # loop with zip to avoid out-of-bounds
            ...
            # raw nearest this frame
            raw_nearest_det, raw_nearest_idx = self.find_nearest_person(detection_objects)

            # smoothing: append raw idx, pick most common
            self.nearest_history.append(raw_nearest_idx)
            counts = Counter(self.nearest_history)
            smoothed_idx, _ = counts.most_common(1)[0]

            # choose smoothed if valid
            if smoothed_idx is not None and smoothed_idx < len(detection_objects):
                nearest_det = detection_objects[smoothed_idx]
                nearest_idx = smoothed_idx
            else:
                nearest_det = raw_nearest_det
                nearest_idx = raw_nearest_idx
            ...
            for idx, det in enumerate(detection_objects):
                ...
                if is_nearest:
                    dist = math.hypot(det['x'] - self.slam_point[0],
                                      det['y'] - self.slam_point[1])
